 ...
 * @returns 
 */
export function toHook(designList) {
    const container = IString()
    const pageFileName = "Example";
    const { list, iModules, iReactBatch, iInfo } = toComponents(designList)
    const doms = renderDoms({ list })
...
function resetInstructFunction(funString = '', { iInfo, getName } = {}) {
    //ref
    ...
    if (loadingDialogIndex !== -1) {
        ...
        funString = resetInstructFunction(
            ...
            + `Dialog.loading(${info.str})` + funString.substring(end + 1),
            { getName, iInfo }
        )
    }
    //$open drawer
    ...
    if (closeDrawerIndex !== -1) {
        ...
        funString = resetInstructFunction(
            ...
            + `Drawer.close(${info.str})` + funString.substring(end + 1),
            { getName, iInfo }
        )
    }
    //$loading drawer
    ...
    if (loadingDrawerIndex !== -1) {
        ...
        funString = resetInstructFunction(
            ...
            + `Drawer.loading(${info.str})` + funString.substring(end + 1),
            { getName, iInfo }
        )
    }
    //$set Data
    ...
    if (setIndex !== -1) {
        ...
        if (info) {
            ...
            funString = resetInstructFunction(funString.substring(0, setIndex) + state + funString.substring(end + 1), { getName, iInfo })
        }
    }
